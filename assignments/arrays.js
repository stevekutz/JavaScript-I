...
// make an alias
const c33 = inventory[33-1]; // 33 is id(starts at 1) but index is 32(starts at 0)
console.log(`Car ${c33.id} is a ${c33.car_year} ${c33.car_make} ${c33.car_model}`);


// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory.length -1 ;
let lastCar_2 = `${inventory[lastCar].car_make} ${inventory[lastCar].car_model}`;
//console.log(`The last car is a ${inventory[lastCar].car_make} ${inventory[lastCar].car_model}`);

console.log(`The last car is a ${lastCar_2}`);
...
for(let i  = 0; i < inventory.length; i++) {
  carModels.push( inventory[i].car_model[0].toUpperCase() + inventory[i].car_model.slice(1));
}
...
for(let i = 0; i < inventory.length; i++){
  ...
  if(make ==='BMW' || make === 'Audi') BMWAndAudi.push(inventory[i]);
}
// JSON.stringify needed to convert the  key: value pairs of each object within array
